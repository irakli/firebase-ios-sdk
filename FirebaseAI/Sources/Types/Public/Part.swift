...
// Xcode 27 beta 5 removed `Transcript.CustomSegment` and the `Transcript.Segment.custom` case, so a
// part can no longer be injected into a transcript as itself. The executor now maps each part onto an
// action the response channel still has: images become attachment segments, code parts become text, and
// anything carrying opaque bytes is recorded in metadata. Nothing here needs a conformance.
